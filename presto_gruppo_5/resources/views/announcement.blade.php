<x-layout>
    ...
    <div class="container justify-content-center mt-5">
        <div class="row text-center">
            <div class="col-12">
                <h1 class="font-primary h1-relative"> {{ __('ui.header') }}</h1>
            </div>
        </div>

        {{-- /categorie --}}

        <div class="rounded swiffy-slider slider-item-show6 slider-nav-outside slider-nav-dark slider-nav-sm slider-nav-visible slider-nav-page slider-item-snapstart slider-nav-autoplay slider-nav-autopause slider-item-ratio slider-item-ratio-contain slider-item-ratio-32x9{{--  bg-white shadow-lg --}} py-3 py-lg-4 mb-5"
            data-slider-nav-autoplay-interval="2000">
            <div class="slider-container">
                @foreach ($categories as $category)
                    <a class="btn btn-category text-center"
                        href="{{ route('article.type', [
                            'category_id' => $category->id,
                        ]) }}"
                        role="button">
                        <div class="mybutton text-center d-flex align-items-center justify-content-center fw-bolder fs-5">
                                {{ $category->name }}
                        </div>
                    </a>
